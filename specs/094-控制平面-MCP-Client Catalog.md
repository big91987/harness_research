...
MCP Client Catalog 解决的问题不是“让模型立刻调用所有外部 MCP 工具”，而是先把 MCP server 的配置、生命周期、握手、工具列表和单工具调用跑成可验证闭环。注册层负责发现和同步能力；进入模型工具池时，还要交给 runtime loading、policy、sandbox 和 trace 继续处理。
...
MCP stdio transport 使用 JSON-RPC 消息。Catalog 实现保持最小 client：每次检查一个 server 时启动进程，发送 `initialize`，再发送 `tools/list` 或 `tools/call`。它只负责资产发现；模型可见工具由 [095-运行时层-MCP-Runtime Loading](095-运行时层-MCP-Runtime%20Loading.md) 注入，并通过权限策略隔离。
...
- 安全边界：当前模块是 catalog/client；runtime 注入由 095 负责，并必须接 policy、sandbox、trace。
...
本仓库先实现 stdio client catalog，是为了把协议握手和工具 schema 同步讲清楚。和产品级 Harness 相比，当前还没有做多 transport、resources/prompts、server 级权限和 MCP tool sandbox wrapper。
...
- 将 MCP runtime loading 继续接入更细的 tool profile。
...
smoke test. Runtime injection is handled by the runtime layer, where policy,
sandbox, and trace can wrap external tools safely.
